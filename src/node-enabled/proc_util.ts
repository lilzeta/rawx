...
const NO: str = `\x1B[0m`;
class Proc_Util implements Proc_Util_I {
    ...
    setup_subproc = ({ sub_proc, silence, on_close }: Setup_Proc_Util_Args) => {
        ...
        if (!silence) {
            ...
            if (o.colors.default?.length) sub_pass_through = () => this.stdout(colors.default);
        ...
        if (silence !== "all" && o.debug) {
            ...
            // no label for proc std_out
            if (colors.errata?.length) err_sub_pass_through = () => this.stdout(colors.errata);
